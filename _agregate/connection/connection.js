...
export class Connection extends Driver {
  ...
  async registerRecordClass(
    ...
    const metadata = await this.__Metadata.firstOrInitialize(
    ...
    for (const key of R.difference(
      metadata.existenceConstraints,
      existenceConstraints
    )) {
      await tx.query(
        C.tag`DROP CONSTRAINT ON (entity:${C.raw(
          label
        )}) ASSERT exists(value.${C.raw(key)})`
      )
    }
    for (const key of R.difference(
      existenceConstraints,
