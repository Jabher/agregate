...
class Test extends Record {
  ...
  static connection = new Connection("localhost", {
    username: "neo4j",
    password: "password"
  })
}
...
describe("Agregate Record", () => {
  before(async () => await Test.register())
  ...
  describe("classes", () => {
    ...
    cleanBeforeEach()
    beforeEach(async () => await (instance = new Test(opts)).save())

    it("should create new record", async () =>
      expect(await Test.byUuid(instance.uuid)).to.deep.include(opts))
    it("should create new record by Record#firstOrInitialize", async () =>
      expect(await Test.firstOrInitialize(opts)).to.deep.include(opts))
    it("should return existing record if exists by Record#firstOrInitialize", async () =>
      expect(await Test.firstOrInitialize(opts)).to.deep.include({
        uuid: instance.uuid
      }))
  ...
  describe("props", () => {
    ...
    it("should save updated props", async () => {
      ...
      await instance.save()
      expect(await Test.byUuid(instance.uuid)).to.deep.include({ ...instance })
    })
  })
  describe("querying", () => {
    const test = "test"
    ...
    describe("basics", () => {
      ...
      it("should reveal item by int prop", async () =>
        expect(await Test.where({ idx: items[0].idx })).to.have.deep
          .property("[0]")
          .that.deep.includes(items[0]))
      it("should support reverse order", async () => {
        const limit = 2
        const result = await Test.where(
          { test },
          { limit: 2, order: "idx DESC" }
        )
      ...
      it("should support limit", async () => {
        ...
        const result = await Test.where(
          { test },
          { limit: 2, offset: 1, order: "idx ASC" }
        )
        expect(result).to.have.length(limit)
    ...
    describe("numbers", () => {
      ...
      it("should support $lt", async () =>
        expect(await Test.where({ test: { $lt: 2 } }), {
          order: "test"
        }).to.have
          .length(1)
      ...
      it("should support $lte", async () =>
        expect(
          await Test.where({ test: { $lte: 2 } }, { order: "test" })
        ).to.have
          .length(2)
          .and.to.have.deep.property("[1].test", 2))
      it("should support $gt", async () =>
        expect(await Test.where({ test: { $gt: 2 } }), {
          order: "test"
        }).to.have
          .length(1)
      ...
      it("should support $gte", async () =>
        expect(
          await Test.where({ test: { $gte: 2 } }, { order: "test" })
        ).to.have
          .length(2)
          .and.to.have.deep.property("[0].test", 2))

      it("should support multiple $lt", async () =>
        expect(await Test.where({ test: { $lt: [2, 3] } }), {
          order: "test"
        }).to.have
          .length(1)
          .and.to.have.deep.property("[0].test", 1))
      it("should support multiple $lte", async () =>
        expect(
          await Test.where({ test: { $lte: [2, 3] } }, { order: "test" })
        ).to.have
          .length(2)
          .and.to.have.deep.property("[1].test", 2))
      it("should support multiple $gt", async () =>
        expect(await Test.where({ test: { $gt: [2, 1] } }), {
          order: "test"
        }).to.have
          .length(1)
          .and.to.have.deep.property("[0].test", 3))
      it("should support multiple $gte", async () =>
        expect(
          await Test.where({ test: { $gte: [2, 1] } }, { order: "test" })
        ).to.have
          .length(2)
          .and.to.have.deep.property("[0].test", 2))
    })
    describe("strings", () => {
      cleanBeforeEach()
      beforeEach(() =>
        Test.save({ test: "abcde" }, { test: "ecdba" }, { test: "foo" })
      )
      it("should support $startsWith", async () =>
        expect(await Test.where({ test: { $startsWith: "abc" } })).to.have
          .length(1)
          .and.to.have.deep.property("[0].test", "abcde"))
      it("should support $endsWith", async () =>
        expect(await Test.where({ test: { $endsWith: "cde" } })).to.have
          .length(1)
          .and.to.have.deep.property("[0].test", "abcde"))
      it("should support $contains", async () =>
        expect(await Test.where({ test: { $contains: "a" } })).to.have.length(
          2
        ))

      it("should support multiple $startsWith", async () =>
        expect(
          await Test.where({ test: { $startsWith: ["abc", "ab"] } })
        ).to.have
          .length(1)
          .and.to.have.deep.property("[0].test", "abcde"))
      it("should support multiple $endsWith", async () =>
        expect(await Test.where({ test: { $endsWith: ["cde", "de"] } })).to.have
          .length(1)
          .and.to.have.deep.property("[0].test", "abcde"))
      it("should support multiple $contains", async () =>
        expect(
          await Test.where({ test: { $contains: ["a", "b"] } })
        ).to.have.length(2))
    })
    describe("general", () => {
      cleanBeforeEach()
      beforeEach(() =>
        Test.save({ test: true }, { test: false }, { test2: "test2" })
      )
      ...
      it("should support falsy $exists", async () =>
        expect(await Test.where({ test: { $exists: false } })).to.have
          .length(1)
    ...
    describe("arrays", () => {
      ...
      it("should support $has", async () =>
        expect(await Test.where({ test: { $has: 1 } })).to.have
          .length(1)
          .and.to.have.deep.property("[0].test2", 1))
      it("should support multiple $has", async () =>
        expect(await Test.where({ test: { $has: [1, 2] } })).to.have
          .length(1)
          .and.to.have.deep.property("[0].test2", 1))
      it("should support $in", async () =>
        expect(await Test.where({ test2: { $in: [1, 1, 1, 5, 5] } })).to.have
          .length(1)
          .and.to.have.deep.property("[0].test2", 1))
      it("should support multiple $in", async () =>
        expect(
          await Test.where({
            test2: { $in: [[1, 1, 1, 5, 5], [1, 2, 2, 4, 4]] }
          })
        ).to.have
          .length(1)
          .and.to.have.deep.property("[0].test2", 1))
    ...
    describe("ORs", () => {
      ...
      it("should match same case", async () =>
        expect(await Test.where([{ test1: 2 }, { test2: 2 }])).to.have
          .length(1)
          .and.to.have.deep.property("[0].test2", 2))
      it("should match different cases", async () =>
        expect(await Test.where([{ test1: 1 }, { test2: 2 }])).to.have.length(
          2
        ))
    })
  })
  ...
  describe("hooks", () => {
    let testRecord
    beforeEach(() => (testRecord = new Test()))
    ...
    it("should support transactions", async () => {
      Object.assign(testRecord, {
        async beforeCreate() {
          expect(
            await Test.where({ state: 1 }, this.connection)
          ).to.have.length(0)
        },
        async afterCreate() {
          expect(
