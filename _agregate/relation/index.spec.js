  ...
  describe("relations", () => {
    ...
    describe("promise-management", () => {
      it("should support promise entry sources", async () => {
        await object.subjects.add(
          await TestSubject.where({ uuid: subject.uuid })
        )
    ...
    describe("querying", () => {
      it("should support empty $relation queries", async () => {
        expect(
          await TestSubject.where({
            $relations: [object.subjects]
          })
        ).to.have.length(0)
      })
    ...
    describe("manipulations", () => {
      beforeEach(async () => await object.subjects.add(subject))
      ...
      it("should resolve objects of subject by props", async () =>
        expect(
          await subject.objects.where({ uuid: object.uuid })
        ).to.have.length(1))
      it("should not resolve objects not of subject by props", async () =>
        expect(
          await subject.objects.where({
            uuid: (await new TestObject().save()).uuid
          })
        ).to.has.length(0))
      it("should resolve intersect objects using Relation#intersect", async () =>
        expect(
          await subject.objects.intersect(object, await new TestObject().save())
        ).to.have.length(1))
      it("should resolve objects of subject by #has", async () =>
        expect(await subject.objects.has(await TestObject.where())).to.be.equal(
          true
        ))
      it("should not resolve wrong objects by #has", async () =>
        expect(
          await subject.objects.has(await TestSubject.where())
        ).to.be.equal(false))
    ...
    describe("deep", () => {
      class TestSourceObject extends Test {
        intermediateObjects = new Relation(this, "rel1", {
          target: TestIntermediateObject
        })
        endObjects = new Relation(this.intermediateObjects, "rel2", {
          target: TestEndObject
        })
      }
      ...
      describe("manipulations", () => {
        ...
        it("should contain endItem", async () =>
          expect(await startObject.endObjects.entries()).to.have.deep.property(
            "[0].uuid",
            endObject.uuid
          ))
    ...
    describe("one-to-one", () => {
      beforeEach(async () => await object.subjects.only(subject))
      ...
      it("should resolve a relation", async () =>
        expect(await object.subjects.only()).to.deep.include({
          uuid: subject.uuid
        }))
    })
  })
