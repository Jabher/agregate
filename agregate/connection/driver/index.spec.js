...
describe('Agregate Driver', () => {
  describe('connectivity', () => {
    ...
    it('should fail on wrong login/password pair', async () => {
      const driver = new Driver('localhost', Driver.basic('neo4j', 'wrong password'));
      try {
        await driver.init;
  ...
  describe('querying', () => {
    ...
    it('should perform query with object and object params', async () => {
      const [[node]] = await driver.query('create (a:Test {foo: "bar"}) return a');
      expect(node).to.deep.equal({
        labels: ['Test'],
        properties: { foo: 'bar' },
        __type: 'node'
    ...
    it('should perform query with relation', async () => {
      const [[rel]] = await driver.query('merge (a:Test)-[r:TestRel {foo: "baz"}]->(b:Test) return r');
    ...
    it('should perform query with relation and data', async () => {
      const [[rel]] = await driver.query(`merge (a:Test {from: true})-[r:TestRel]->(b:Test {to: true}) 
            return r, a, b`);
      expect(rel).to.deep.equal({
        labels: ['TestRel'],
        properties: {},
    ...
    it('should perform resolve trick with relations', async () => {
      driver.resolveRelation = (val) => ({ isRelation: true, foo: val.properties.foo });
      const [[rel]] = await driver.query('merge (a:Test)-[r:TestRel {foo: "baz"}]->(b:Test) return r');
      expect(rel).to.deep.equal({
        start: undefined,
        end: undefined,
