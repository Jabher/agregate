...
export class BaseRecord {
  ...
  get __unassignableProperties(): string[] { return this.constructor.__unassignableProperties }
  ...
  static async register() {
    //$FlowFixMe
    const uniquenessConstraints: string[] = this.unique.filter(value => typeof value === 'string');
    //$FlowFixMe
    const nodeKeyConstraints: string[][] = this.unique.filter(prop => Array.isArray(prop));
  ...
  get __isReflected(): boolean { return this.__node !== undefined }

  get __node(): neo4j.types.Node { return reflections.get(this); }

  set __node(node: ?neo4j.types.Node): void { node ? reflections.set(this, node) : reflections.delete(this); }

  get uuid(): ?string {
    if (this.__properties)
      return this.__properties.uuid;
  }

  get createdAt(): ?number {
    if (this.__properties)
      return this.__properties.createdAt;
  }
